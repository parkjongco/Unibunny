	...
	$.ajax({
		...
		for (let i of resp) {
			...
			if (true) {
				...
				edit_btn.text("수정");
				let submit_btn = $("<button>", { "class": "write_btn edit_btn", "type": "submit" });
				submit_btn.text("완료");
				btn_box1.append(edit_btn, submit_btn);

				let btn_box2 = $("<div>", { "class": "btn_box" });
				let del_btn = $("<button>", { "class": "write_btn comm_btn", "type": "button" });
				del_btn.text("삭제");
				let cancel_btn = $("<button>", { "class": "write_btn edit_btn", "type": "button" });
				cancel_btn.text("취소");
			...
			let option_btn = $("<button>", { "class": "option_btn likes_option", "type": "button" });
			let icon1 = $("<i>", { "class": "fa-regular fa-thumbs-up fa-xs option_icon" });
			let icon2 = $("<i>", { "class": "fa-solid fa-thumbs-up fa-xs option_icon", "style": "display:none" });
			let likes_num = $("<p>");
			likes_num.text(i.thumbs_up);
